...
# A Herdr spawn uses one project-owned primary workspace or one stable marked
# second-mate-home workspace, with one tab per task, as owned by
# bin/backends/herdr.sh and docs/herdr-backend.md. Both branches converge on
# the same $T ("target") string
      ...
      if [ -e "$HERDR_PRESENTATION_JOURNAL" ] || [ -L "$HERDR_PRESENTATION_JOURNAL" ]; then
        ...
        if [ "${HERDR_RECOVERY_BACKEND:-}" = herdr ]; then
          ...
          FM_HOME="$HERDR_LABEL_HOME" fm_backend_herdr_projection_reclaim_task \
            ...
            "$HERDR_PARENT_LABEL" "$KIND" "$HERDR_TASK_LABEL" "$PROJ_ABS"
          ...
          case "$HERDR_RECLAIM_STATUS" in
            0)
              ...
              HERDR_WORKSPACE_LABEL=$FM_BACKEND_HERDR_JOURNAL_WORKSPACE_LABEL
              HERDR_TAB_LABEL=$HERDR_TASK_LABEL
              HERDR_PANE_LABEL=$HERDR_TASK_LABEL
          ...
          if [ -z "$HERDR_PARENT_WORKSPACE_ID" ]; then
            ...
            if [ -n "$HERDR_HOME_ID" ] \
               ...
               && fm_backend_herdr_projection_journal_bind \
                 "$HERDR_PRESENTATION_JOURNAL" "$ID" "$KIND" "$HERDR_HOME_ID" "$HERDR_SES" \
                 "$HERDR_WORKSPACE_ID" "$HERDR_TAB_ID" "$HERDR_PANE_ID" \
                 "$HERDR_PARENT_WORKSPACE_ID" "$HERDR_PARENT_LABEL" \
                 "$HERDR_PROJECTION_LABEL" "$HERDR_TASK_LABEL"; then
              :
            else
