...
family_for_basename() {
  case "$1" in
    ...
    fm-herdr-session-cleanup.test.sh|fm-rollover.test.sh|fm-send-strict.test.sh|\
    fm-spawn-batch.test.sh|fm-spawn-dispatch-profile.test.sh|fm-spawn-worktree-settle.test.sh)
      printf '%s\n' backend-dispatch
      ;;
...
families_for_changed_path() {
  ...
  case "$path" in
    ...
    bin/fm-x-*|bin/fm-check*)
      printf '%s\n' pr-forge
      ;;
    bin/fm-spawn.sh|bin/fm-rollover.sh|bin/fm-send.sh|bin/fm-harness.sh|\
