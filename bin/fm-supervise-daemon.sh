...
# from a normal keyboard at the start of a message and terminal input transports
# as text. Firstmate's contract: a message that starts with the marker is an
# internal escalation (stay afk); a message without it means the captain is back
# (exit afk, flush catch-up, resume per-wake responsiveness).
...
# ASCII unit separator, terminal input transports U+2063 as text rather than
# consuming it as a control action. Every daemon injection is prefixed with this
# character; firstmate treats a leading marker as an internal escalation (stay
# afk) and its absence as "captain is back" (exit afk). Portable across harnesses:
# it travels with the message text, independent of any harness-level typed-vs-
# injected distinction.
