...
# binary. Skip cleanly when it is absent or incompatible (matching the backend
# smoke suites); the handoff path itself requires atomic multi-ID mv (0.2.2+).
...
fm_tasks_axi_compatible || { echo "skip: installed tasks-axi lacks atomic multi-ID mv support (0.2.2+), required by the delegated handoff path"; exit 0; }
