...
phase_handoff() {
  ...
  # incompatible - the handoff path requires atomic multi-ID mv (0.2.2+) - since
  ...
  if ! fm_tasks_axi_compatible; then
    echo "skip: installed tasks-axi lacks atomic multi-ID mv support (0.2.2+; backlog handoff delegates to it)"
    return 0
  fi
