...
Pass `--build` when the task creates or changes reusable machinery, so the launch carries the harness's stock Superpowers activation from `bin/fm-build-method.sh`; a Run task carries none.
...
Deciding a task needs a workflow is firstmate's call, made at intake: a task is a workflow when its work divides into ordered phases and one of those phases fans out across parallel agents whose results must be verified and then synthesised.
Scaffold that task with `--workflow` and supply the shape - phases, fan-out, verification, synthesis - because the scaffold refuses a blank or placeholder shape and a crewmate cannot build a shape it was never given.
