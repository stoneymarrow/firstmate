...
**Always-on wake triage (absorb only with positive evidence).**
`bin/fm-watch.sh` classifies every wake in bash and absorbs the benign majority without waking you: crews with positive working evidence (an actively-running no-mistakes step for their branch, or a busy pane, read via `bin/fm-crew-state.sh`), stale panes in legitimate idle waits until their bounded recheck cadence, and no-change heartbeats.
