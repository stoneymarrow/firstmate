...
tests/fm-brief.test.sh                    # fm-brief.sh bash -n and complete-help regression guards, clean delivery-mode and configured-pause scaffolds, default and task-specific stop conditions, budget validation, and hard Herdr lifecycle declaration/contract tests
...
tests/fm-backend.test.sh                  # runtime-backend abstraction: fm-backend.sh selection/meta/dispatch helpers, shell-portable sourced backend matching, blocked codex-app refusal, transient worktree-readiness handling, and old-vs-new fake-tool command-log conformance for fm-send/fm-peek/fm-spawn/fm-teardown
