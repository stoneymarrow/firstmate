...
| Duplicate task check | `herdr tab list --workspace <id>`, match the stable `fm-<id>` label or the prior metadata-recorded tab id | Herdr does NOT enforce tab-label uniqueness itself; two tabs can share a label. The adapter creates under the stable identity, recognizes a previously human-labeled tab through its recorded `herdr_tab_id=`, and applies the display rename only after task metadata is durable. |
