...
| Tab rename | `herdr tab rename <tab_id> <label>` | Renames the tab and returns the updated tab object in `.result.tab`, with `fm_backend_herdr_rename_task` resolving the current tab id from the live pane first. |
