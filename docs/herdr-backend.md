...
| Recovery / list-live | `herdr tab list --workspace <id>`, map exact metadata-recorded tab ids back to `fm-<id>` and preserve legacy labels starting with `fm-` | Human-readable labels are display-only, so an unrecorded manual colon tab is never classified as a Firstmate task. `<id>` is always THIS home's own workspace (`fm_backend_herdr_workspace_find`), so recovery never sees a sibling home's tabs. |
...
Practical consequence: stored `herdr_tab_id=` and `herdr_pane_id=` values remain valid identity and operational targets across an ordinary server restart within the same named session, regardless of how many other homes' workspaces coexist in that session.
`fm_backend_herdr_list_live` maps metadata-recorded tab ids back to stable `fm-<id>` identities for human-labeled tabs and retains label-based discovery only for tabs whose visible labels already carry the legacy `fm-` identity marker.
