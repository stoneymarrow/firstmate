...
FM_CREW_STATE_BIN=bin/fm-crew-state.sh   # test override for the current-state reader used by working/idle-wait watcher triage
...
FM_STALE_ESCALATE_SECS=240         # idle seconds before a provably-working stale pane escalates; stale panes outside a legitimate idle wait surface immediately when the crew is not provably working
FM_PAUSE_RESURFACE_SECS=3600       # seconds before a legitimate idle wait re-surfaces in the watcher, or a declared external wait re-surfaces in the away-mode daemon
