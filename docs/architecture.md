...
Herdr's durable default container shape is workspace-per-home plus tab-per-task, with adapter-owned project/task and role labels that corroborate exact home-local metadata tuples.
The named session, workspace, tab, and pane ids remain the sole routing, liveness, capture, send, and cleanup targets; display labels never select an object.
Herdr task and second-mate parent records carry an honest display-only shared-session alias because the native session cannot be renamed and can span several homes.
New Herdr task metadata publishes through a validated same-directory temporary file and rename, while the default metadata path for every other backend remains unchanged.
The six captain-facing readers render Herdr labels first without changing `fm-fleet-snapshot.v1`, `endpoint.target`, crew-state's leading grammar, successful-send silence, or piped-peek bytes.
The already-running native primary tuple has a separate process-ownership owner in `bin/fm-herdr-primary-labels.sh`; locked session start invokes it before ordinary bootstrap, while read-only startup and marked secondmate homes leave it inert.
That owner binds the home session-lock pid to this harness ancestry, holds the shared socket-keyed Herdr session lock, and verifies the exact Herdr socket and environment ids, the pane's foreground process row and physical Firstmate-root cwd, accepted old/new labels, and global target-label uniqueness before exact-ID-only rename.
