...
Herdr is experimental and keeps Treehouse as its worktree provider.
Exact home-local metadata ids remain the routing and lifecycle authority, while labels only corroborate those ids or improve display; the native primary tuple stays under a separate process-ownership owner and never gains a task ownership record.
[`configuration.md`](configuration.md#runtime-backend-configbackend--fm_backend) owns the metadata schema, [`herdr-backend.md`](herdr-backend.md) owns current setup, behavior, and safety limits, and [`verification/runtime-backends.md`](verification/runtime-backends.md#herdr) owns active evidence.
