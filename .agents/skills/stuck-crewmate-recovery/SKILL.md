...
4. If the crewmate is genuinely wedged after redirection, exit the agent with the adapter's exit command and relaunch with the same brief plus a `progress so far` note appended to it.
   ...
   A worker stopped at its context ceiling is a different case with its own owner: if its status carries the decision key `context-rollover`, stop here and load `context-rollover` instead of relaunching from this playbook.
